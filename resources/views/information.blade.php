                    ...
                    @foreach ($information as $info)
                        <article class="text-content flex-container"> 
                            <div class="text-content news-item">
                                <h3>{{ $info['title'] }}</h3>
                                <p class="text-muted mb-0">Ditulis {{ $info['tanggal'] }}</p>
                                <p>{{ Str::limit(strip_tags($info['isi']), 250) }}</p>
                                <a href="{{ url('/manyaran/information/' . $info['slug']) }}" class="read-more">Selengkapnya</a>
                            </div>
                        </article>
                    @endforeach
